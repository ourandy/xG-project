...
class PostSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://fbref.com/en/comps/11/1640/possession/2017-2018-Serie-A-Stats",
    ]

    def parse(self, response):
        columns = []
        for column_node in response.xpath(
            '//*[@id="stats_possession_squads"]/thead/tr[2]/th'
        ):
            column_name = column_node.xpath("./text()").extract_first()
            columns.append(column_name)

        matches = []
        for row in response.xpath('//*[@id="stats_possession_squads"]/tbody/tr'):
            match = {}
            suffixes = {}
        ...
        yield df.to_csv("serie_a_possession_squads.csv", sep=",", index=False)
